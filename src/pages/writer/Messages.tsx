...
const Messages = () => {
  ...
  const fetchConversations = useCallback(async () => {
    ...
    const enriched = await Promise.all(
      convos.map(async (conv) => {
        ...
        return {
          ...conv,
          producer_name: profile?.name || "Producer",
          producer_avatar: profile?.avatar_url || null,
    ...
    const grouped = sortByRecent<ConversationGroup>(
      enriched.reduce<ConversationGroup[]>((groups, conversation) => {
        const existingGroup = groups.find((group) => group.producer_id === conversation.producer_id);

        if (!existingGroup) {
          groups.push({
            id: conversation.producer_id,
            conversation_ids: [conversation.id],
            producer_id: conversation.producer_id,
            producer_name: conversation.producer_name,
            producer_avatar: conversation.producer_avatar,
  ...
  const sendMessage = async (overrideContent?: string) => {
    ...
    await trackEvent({
      ...
      metadata: {
        conversation_id: targetConversationId,
        producer_id: selectedGroup.producer_id,
        producer_name: selectedGroup.producer_name,
  ...
  const handlePitchAction = (action: "view" | "discuss" | "accept", pitch: PitchCardData) => {
    ...
    if (action === "discuss") {
      setNewMessage(`I'd love to discuss "${pitch.title}" further. Can we talk through tone, audience, and next steps?`);
      return;
    }

    setNewMessage(`I'm excited about "${pitch.title}". Let's move forward with the pitch and discuss availability.`);
  };

  const filteredConversations: MessagingConversationItem[] = conversations
    .filter((conv) =>
      conv.producer_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      conv.script_title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      conv.last_message?.toLowerCase().includes(searchTerm.toLowerCase()),
    )
    .map((conv) => ({
      id: conv.id,
      name: conv.producer_name,
      avatar: conv.producer_avatar,
      scriptTitle: conv.script_title,
      unreadCount: conv.unread_count,
      lastMessageAt: conv.last_message_at,
      lastMessage: conv.last_message,
      roleLabel: "Producer",
      status: conv.unread_count > 0 ? "Active now" : "Last seen recently",
    }));
  ...
  return (
    <WriterLayout forceDark hideThemeToggle>
      <MessagingWorkspace
        ...
        isTyping={isTyping}
        typingLabel={`${filteredConversations.find((conversation) => conversation.id === selectedConversation)?.name || "Producer"} is typing...`}
        emptySidebarTitle="No conversations yet"
        emptySidebarDescription="Messages from producers will appear here."
        emptyChatTitle="Select a conversation"
        emptyChatDescription="Choose a producer from the sidebar to continue discussing your screenplay."
        panelLabel="Writer Inbox"
        theme={theme}
